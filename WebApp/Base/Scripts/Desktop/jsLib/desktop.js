...
myLib.desktop.win = {
    ...
    closeWin: function (obj) {
        ...
        obj.hide(200, function () {
            $.each($(this).find("iframe"), function (i, ifrm) {
                destroyIframeMemory(ifrm);
            });
            $(this).remove();
        });
    ...
    newWin: function (options) {
        ...
        //options['iframSrc'] = $.trim(options['iframSrc']) != "" 
        //判断如果此id的窗口存在，则不创建窗口
        if ($("#" + id).size() <= 0) {
            //在任务栏里添加tab
...
myLib.desktop.lrBar = {
    ...
    init: function (iconData) {
		...
		.click(function () {
		    ...
		    $(this).data("currPanel", $("div.currDesktop").index("div.deskIcon"));
        ...
        $newTab
			...
			.click(function () {
			    var win = $("#myWin_" + $(this).data('win')),
						...
						desk = $("#" + iconId).parent(),
 						i = desk.index("div.deskIcon"); //判断窗口在那个桌面区域
			    ...
			    if (win.is(".hideWin")) {
			        ...
			        if (win.hasClass("ui-state-disabled")) {
			            $("#desktopPanel").css("z-index", 95);
			        }
			    } else {
...
myLib.desktop.deskIcon = {
    delIcon: function (id) {
        var _this = this;
        mini.confirm("确认删除?", "确认", function (action) {
            if (action == "ok") {
                addExecuteParam("ID", id);
                execute("/Base/PortalBlock/ShortCut/Delete", {
                    onComplete: function () {
                        if ($("#" + id).size() > 0) {
                            $("#" + id).remove();
                            _this.init();
    ...
    addIcon: function (data) {
        ...
        if (!$("#" + data.id).size()) {
            ...
            $(iconHtml).insertBefore("div.currDesktop .add_icon");
            _this.init();
    ...
    arrangeIcons: function (desktop) {
        ...
        var rows = Math.floor((winWh['h'] - navBarHeight - 0) / gH);
        var cols = Math.ceil(iconNum / rows);
        var curcol = 0, currow = 0;

        var icons = desktop.find("li");
        var arrIDs = [];
        $.each(icons, function (i, item) {
            if ($.trim($(item).attr("id")) != "" && !($(item).hasClass("add_icon"))) {
                arrIDs.push($(item).attr("id"));
            }
        });
        var ids = arrIDs.join(',');
        if (typeof (desktop.data("iconIDs")) == "undefined") {
            desktop.data("iconIDs", ids);
        }
        else {
            if (desktop.data("iconIDs") != ids) {
                addExecuteParam("PageIndex", desktop.attr("pageindex"));
                addExecuteParam("IDs", ids);
                execute("/Base/PortalBlock/ShortCut/SortShortCut", {
                    onComplete: function () {
                        desktop.data("iconIDs", ids);
                    }
                });
    ...
    upDesktop: function ($deskIcon, $deskIconBlock, $innerPanel, $deskIconNum, navBarHeight, lBarWidth) {
        var myData = myLib.desktop.getMydata()
		    , winWh = myData.winWh
		    , w = winWh['w'] - lBarWidth
		    , h = (winWh['h'] - 0 - navBarHeight)
		    , _this = this;

        //设置桌面图标容器元素区域大小
        $innerPanel.css({ "width": w + "px", "height": h + "px" });
        $deskIcon.css({ "width": w + "px", "height": h + "px" });
        $deskIconBlock.css({ "width": w + "px", "height": h + "px", 'margin-left': lBarWidth + 'px', 'margin-bottom': 0 + "px" });

        $deskIcon.each(function (i) {
            _this.arrangeIcons($(this));
            var $icon = $($deskIcon[i]);
            $icon.css("top", "0");
            if ($icon.hasClass("currDesktop")) {
                $icon.css("left", 0);
            }
            else {
                $icon.css("left", winWh['w']);
            }
    ...
    desktopMove: function ($innerPanel, $deskIcon, $navTab, dates, moveDx, nextIndex) {
        ...
        //							.addClass("currTab");
        //		 });
        var curIndex = $deskIcon.filter(".currDesktop").index();
        //左移
        if (curIndex > nextIndex) {
            $deskIcon.filter(".currDesktop").animate({ left: moveDx }, dates);
            $deskIcon.removeClass("currDesktop").eq(nextIndex).addClass("currDesktop").css("left", 0 - moveDx).animate({ left: 0 + $innerPanel.position().left }, dates);
        }
        else if (curIndex < nextIndex) {
            $deskIcon.filter(".currDesktop").animate({ left: 0 - moveDx });
            $deskIcon.removeClass("currDesktop").eq(nextIndex).addClass("currDesktop").css("left", moveDx).animate({ left: 0 + $innerPanel.position().left }, dates);
        }
    ...
    init: function (iconData) {
        ...
        myLib.desktop.iconDataInit(iconData);
        //给添加按钮附件data数据
        $.each($deskIcon.find("li.add_icon"), function (i, item) {
            var $deskIcon = $(item).parent();
            if ($deskIcon.hasClass("deskIcon")) {
                $(item).data("iconData", {
                    'title': "添加快捷方式",
                    'url': '/Base/Desktop/Home/AddIcon?PageIndex=' + $deskIcon.attr("pageindex"),
                    'winWidth': 700,
                    'winHeight': 530
                })
            }
        ...
        },
        {
            text: "删除快捷方式",
            func: function () {
                myLib.desktop.deskIcon.delIcon(this.id);
            }
        }]],
        myLib.desktop.contextMenu($deskIcon.find("li").not("li.add_icon"), i, "icons", 10);
    }
}
/*----------------------------------------------------------------------------------	
//声明插件空间，插件相关js操作
----------------------------------------------------------------------------------*/
myLib.NS("desktop.plugin");
myLib.desktop.plugin = {
    init: function (pluginData) {
        var _this = this;
        var myData = myLib.desktop.getMydata()
        $.each(pluginData, function (i, item) {
            _this.add(item, myData);
        });
        $("body").data("pluginData", pluginData);
    },
    add: function (pluginObj, myData) {
        if (typeof (myData) == "undefined") {
            var myData = myLib.desktop.getMydata();
        }
        var _this = this;
        if (typeof (pluginObj) != "undefined" && pluginObj && pluginObj.IsUse == "T") {
            pluginObj.id = _this.getPluginID(pluginObj.key);
            if ($("#" + pluginObj.id).length == 0) {
                var $innerTitle = $("<div></div>").addClass("innerPluginTitle").append($("<b></b>").text($.trim(pluginObj.name))).append($("<span></span>").addClass("PluginBtn").append($("<a></a>").attr("href", "javascript:void(0)").addClass("PluginClose").attr("title", "删除").click(function () { _this.remove(pluginObj.key) })));
                var $divTitle = $("<div></div>").addClass("PluginTitle").append($innerTitle);

                var $ifm = $("<iframe></iframe>").attr("src", pluginObj.url).attr("allowtransparency", true).attr("frameborder", "0").width("100%").height("100%");
                var $con = $("<div></div>").addClass("PluginCon").height(pluginObj.height).append($ifm);
                //var $removeIcon = $("<div></div>").attr("title", "删除").addClass("plugin_remove").click(function () { _this.remove(pluginObj.key) }).hide();
                var $divContainer = $("<div></div>").addClass("Plugin").attr("id", pluginObj.id).width(pluginObj.width).height(pluginObj.height + 30).css("right", 61).css("top", 80).append($divTitle).append($con);
                $divContainer.hover(
                function () {
                    $(this).find(".innerPluginTitle").show();
                },
                function () {
                    $(this).find(".innerPluginTitle").hide();
                });
                var $curDesktop = myData.panel.desktopPanel.innerPanel.find("div.currDesktop");
                $curDesktop.append($divContainer);
                dragPrototype($divContainer[0]);
                $divContainer.data("pluginData", pluginObj);
            }
            else {
                var pluginDesktop = $("#" + pluginObj.id).parent();
                if (!pluginDesktop.hasClass("currDesktop")) {
                    var aPlugin = myData.panel.navBar.find("a:eq(" + pluginDesktop.attr("pageindex") + ")");
                    if (aPlugin.length > 0) {
                        aPlugin.click();
                    }
                }
    ...
    remove: function (pluginKey) {
        var _this = this;
        var data = _this.getData();
        $.each(data, function (i, item) {
            if (item.key == pluginKey) {
                data[i].IsUse = "F";
                var id = _this.getPluginID(pluginKey);
                $("#" + id).remove();
                return;
            }
        });
    },
    getPluginID: function (pluginKey) {
        return pluginKey + "_plugin";
    },
    getData: function () {
        return $("body").data("pluginData");
    }
}
...
function dragPrototype(dragControl, dragContent) {
    ...
    dragControl.onmousedown = function (e) {
        ...
        document.onmouseup = function () {
            _drag = false;
            document.body.releaseCapture && dragContent && dragContent.releaseCapture();
        };
    };
}

function aboutUS() {
    window.open("http://www.goodwaysoft.com", "_blank");
}

function logout() {
    window.top.isStop = false;
    window.top.open("/Portal/LoginBegin.aspx?t=" + Math.random(), "_self");
}
