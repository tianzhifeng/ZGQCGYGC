...
{
    public class ComprehensiveFormController<T> : BaseAutoFormController where T : class, new()
    {
        SQLHelper sqlDB;
        public virtual SQLHelper ComprehensiveSQLDB
        {
            get
            {
                if (sqlDB == null)
                    sqlDB = SQLHelper.CreateSqlHelper(ConnEnum.Comprehensive);
                return sqlDB;
            }
        }

        public virtual DbContext ComprehensiveDbContext
        {
            get
            {
                return FormulaHelper.GetEntities<ComprehensiveEntities>();
            }
        }
