...
var actTab, currentRow, rowValue;
var para = getUrlParam("para");
function pageLoad() {
...
function addRow(t) {
    ...
    if (t == 1) {
        var newRowVal = "{fid:'" + (!currentRow ? "'" : currentRow.record.ID + "'") + defaultVal(grid) + "}";
        ...
        var newRowVal = "{fid:'" + (!currentRow ? "'" : currentRow.record.ID + "'") + defaultVal(grid_tabs) + "}";
        var newRow = eval('(' + newRowVal + ')');
...
function saveList(t) {
    ...
    $.ajax({
        url: postUrl,
        data: { data: json, fid: para || t == 1 ? grid.fid : mini.get(settings.id).fid },
        type: "post",
        success: function (text) {
            msgUI("保存成功");
            t == 1 ? grid.reload() : mini.get(settings.id).reload();
        },
        error: function (jqXHR, textStatus, errorThrown) {
