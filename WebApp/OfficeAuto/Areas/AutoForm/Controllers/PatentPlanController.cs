...
namespace OfficeAuto.Areas.AutoForm.Controllers
{
    ...
    /// 专利计划
    /// </summary>
    public class PatentPlanController : OfficeAutoFormContorllor<T_Patent_Plan>
    {
        ...
        protected override void OnFlowEnd(T_Patent_Plan entity, Workflow.Logic.Domain.S_WF_InsTaskExec taskExec, Workflow.Logic.Domain.S_WF_InsDefRouting routing)
        {
            if (entity != null)
            {
        ...
        public JsonResult ValidateUpVersion(string Data)
        {
            ...
            if (plan == null) throw new Formula.Exceptions.BusinessException("没有找到指定对的专利计划，不能进行变更操作");
            if (plan.FlowPhase != "End" || String.IsNullOrEmpty(plan.RelateID))
                throw new Formula.Exceptions.BusinessException("不能对没有审批完成的专利计划进行变更操作");

            string FlowVersionNumberStart = "1";
            if (!string.IsNullOrEmpty(System.Configuration.ConfigurationManager.AppSettings["FlowVersionNumberStart"]))
                FlowVersionNumberStart = System.Configuration.ConfigurationManager.AppSettings["FlowVersionNumberStart"];

            string sql = string.Format("select ID,VersionNumber,FlowPhase from {0} where 1=1 and RelateID='{1}'", "T_Patent_Plan", plan.RelateID);
