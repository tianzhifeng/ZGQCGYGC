...
{
    public class OfficeAutoFlowController<T> : MvcAdapter.BaseFlowController<T> where T : class, new()
    {
        ...
        public override System.Web.Mvc.JsonResult GetModel(string id)
        {
            ...
            if (entity == null)
            {
                entity = this.CreateEmptyEntity<T>();
                isNew = true;
            }
    ...
    //        if (entity == null) tOfficeAutoow new Formula.Exceptions.BusinessException("未能找到ID为【" + id + "】的校审信息记录，无法执行校审");
    ...
    //            {
    //                tOfficeAutoow new Formula.Exceptions.BusinessException("有会签在进行中，不可执行此操作，请等待会签完成。");
    //            }
    //        }
    //        //执行会签
    ...
    //        if (activity == null) tOfficeAutoow new Formula.Exceptions.BusinessException("未能找到ID为【" + ActivityID + "】的活动对象，无法进行校审任务");
    ...
    //    {
    //        if (coSign == null)
    //            tOfficeAutoow new Formula.Exceptions.BusinessException("未找到对应的会审信息。");
    ...
    //                tOfficeAutoow new Formula.Exceptions.BusinessException("未找到会审人员。");
