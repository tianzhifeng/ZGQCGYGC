...
{
    public class OfficeAutoFormContorllor<T> : BaseAutoFormController where T : class, new()
    {
        SQLHelper sqlDB;
        public virtual SQLHelper SQLDB
        {
            ...
            {
                if (sqlDB == null)
                    sqlDB = SQLHelper.CreateSqlHelper(ConnEnum.OfficeAuto);
                return sqlDB;
            }
        }
        ...
        public override DbContext BusinessEntities
        {
            get
            {
                return FormulaHelper.GetEntities<OfficeAutoEntities>();
            }
        }
