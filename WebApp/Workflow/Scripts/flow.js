...
(function ($) {
    ...
    //后台方法路径
    var urlBase = "/Workflow/DefFlow/";
    if (window.location.pathname.indexOf("/Workflow/InsDefFlow/") >= 0)
        urlBase = "/Workflow/InsDefFlow/";

    var myflow = {};
    var isClick = false; //是否已点击过
    var curLineID = ''; //选中线
    myflow.config = {
        editable: getQueryString("editable") == 1 ? true : false,
        moving: {
            flag: false,
            prepdot: { x: 0, y: 0 },
            dots: [],
    ...
    myflow.rect = function (o, r, id, guid) {
    ...
    _ox, _oy; // 拖动时，保存起点位置;
        ...
        $([_rect.node, _text.node, _name.node, _img.node]).bind('click',
            function () {
                ...
                isClick = true;
                showActivitySetting(!_o.ID ? "" : _o.ID, _text.node);
    ...
    myflow.path = function (o, r, from, to, guid, dots) {
        var _this = this, _r = r, _o = $.extend(true, {}, myflow.config.path), _path, _markpath, _arrow, _text, _textPos = _o.text.textPos, _ox, _oy, _from = from, _to = to, _id = 'path'
            + myflow.util.nextId(), _dotList, _autoText = true; _o.lineID = guid;
        ...
        _markpath = _r.path(_o.attr.path.path)
            .attr({ fill: "none", stroke: "white", "stroke-miterlimit": 10, "stroke-width": 14, "-webkit-tap-highlight-color": "rgba(0, 0, 0, 0)", "visibility": "hidden", "pointer-events": "stroke", "cursor": "pointer" });
        ...
        var clickHandler = function (e, src) {
            ...
            if (src && src.getId() == _id) {
                ...
                isClick = true;
                curLineID = _o.lineID;
                showRuleSetting(!_o.lineID ? "" : _o.lineID, _text.node);
        ...
        this.toJson = function () {
            var data = "{lineID:'" + (!_o.lineID ? "" : _o.lineID) + "',from:'" + _from.getId() + "',to:'" + _to.getId()
                ...
                + _text.node.textContent + "',textPos:{x:"
    ...
    function DeleteNodes(_r, c) {
        var ids = acitivtyIDs["ID"];
        $.ajax({
            url: urlBase + "DeleteActivity?acitivtyIDs=" + ids,
            data: { data: "" },
            type: "post",
            success: function (text) {
                ...
                $("#myflow_saveHide").trigger("click", [ids, this]);
            },
    ...
    myflow.init = function (c, o) {
        ...
        */
            ...
            if (arg.keyCode == 46) {
                ...
                if (c) {
                    if (c.getId().substring(0, 4) == 'rect') {
                        if (confirm("确定要删除选中的节点吗？") == true) {
                            DeleteNodes(_r, c);
                        }
                        $("#pointer").click();
                    } else if (c.getId().substring(0, 4) == 'path') {
                        $(_r).trigger('removepath', c);
                        addExecuteParam("lineID", curLineID);
                        execute(urlBase + "DeleteDefRouting", {
                            onComplete: function (result) {
                            },
                            onError: function (data) {
                            }
                        });
                        $("#myflow_saveHide").trigger("click", ["", this]);
                    }
        ...
        $(_r).bind('addrect', function (e, type, o, id) {
            addExecuteParam("defineID", getQueryString("ID"));
            execute(urlBase + "IsExistBeginNode", {
                onComplete: function (r) {
                    if (r > 0 && type == 'start') {
                        msgUI("开始节点只能存在一个!");
                    } else {
                        addExecuteParam("defineID", getQueryString("ID"));
                        addExecuteParam("name", id);
                        addExecuteParam("type", type == 'start' ? "Inital" : (type == 'end' ? "Completion" : "Normal"));
                        execute(urlBase + "NewActivity", {
                            onComplete: function (result) {
                                if (result != "Error") {
                                    var rect = new myflow.rect($.extend(true, {},
                                            myflow.config.tools.states[type], o), _r, id, result)
        ...
        var addpathHandler = function (e, from, to, dots) {
            addExecuteParam("defineID", getQueryString("ID"));
            addExecuteParam("beginActivityID", getNodeID(from));
            addExecuteParam("endAcitivityID", getNodeID(to));
            addExecuteParam("name", "送" + to.text());
            execute(urlBase + "NewRule", {
                onComplete: function (result) {
                    if (result != "Error") {
                        var path = new myflow.path({}, _r, from, to, result, dots);
        ...
        if (myflow.config.editable) {
            ...
            $('#myflow_cloth').click(function () {// 发布
                saveAll(getJson(), true);
                if (getQueryString("IsFlowNav")) {
                    msgUI("发布成功!");
                }
            });

            $('#myflow_saveHide').click(function (event, ids, b) {// 保存
                addExecuteParam("defFlowID", getQueryString("ID"));
                addExecuteParam("json", getJson());
                execute(urlBase + "SaveFlowLayOut", {
                    onComplete: function () {
                    }
                });
            });
